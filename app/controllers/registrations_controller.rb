...
class RegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    user_params.to_h.map {|m,i| @hotel_ids.push(i) }
    p "BROLYYYYYYYYYYYYYYYYYYYYYYYYY"
    p @hotel_ids
    if resource.save
    # resource.hotels_users.create(hotels_user_params)
    @hotel_ids.each do |hotel_id|
			resource.hotels_users.create(hotel_id: hotel_id)
		end
    ...
    if resource.persisted?
      if resource.active_for_authentication?
        ...
        respond_with resource, location: '/v2/hotels/#{@hotel_ids.first.hotel_ids}/users/sign_up'
      end
    else
...
private
  ...
  def user_params
		params.require(:user).permit(:hotel_ids);
	end
